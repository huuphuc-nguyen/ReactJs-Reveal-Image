...
function App() {
  ...
  const [revealSize, setRevealSize] = useState()
  const deviceType = useDeviceType()

    useEffect(() => {
    setCurrentSetImage(images[0])
    setDimensions(data[0])
    setRevealSize(200)
  }, [])

  const images = deviceType === 'desktop' ? [
  ...
  const handleOnChange = (e) => {
    ...
    setCurrentSetImage(images[selectedId])
    setDimensions(data[selectedId])
  }
  ...
  return (
    <>
      <Alert />
      <main className='w-full h-lvh grid place-items-center bg-fuchsia-100'>
